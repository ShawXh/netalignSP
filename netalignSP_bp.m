...
distA = graphallshortestpaths(sparse(A));
distB = graphallshortestpaths(sparse(B));
...
indicesL = sparse(Le(:,1),Le(:,2),1:length(Le(:,1)),size(L,1),size(L,2));
idxA = sparse(Ae(:,1),Ae(:,2),1:length(Ae(:,1)),size(A,1),size(A,2));
idxB = sparse(Be(:,1),Be(:,2),1:length(Be(:,1)),size(B,1),size(B,2));

neighborsL = findNeighbors(indicesL);
neighborsInverseL = findNeighborsInverse(indicesL);
indicesA = compressIndices(idxA);
indicesInverseA = compressIndicesInverse(idxA);
indicesB = compressIndices(idxB);
indicesInverseB = compressIndicesInverse(idxB);

clear idxA;
clear idxB;

indicesL = full(indicesL);
...
while iter<=maxiter
    ...
    for ii = 1:nedgesL
        i = Le(ii,1);
        iapos = Le(ii,2);
        neighSum = neighborSum(i,mpijii(:,ii),indicesA)+neighborSum(i,mpjiii(:,ii),indicesInverseA)...
                    +neighborSum(iapos,mqijii(:,ii),indicesB)+neighborSum(iapos,mqjiii(:,ii),indicesInverseB);
...
function omp=maxsigma(i,j,iapos,lw,indicesL,dist,N)
...
function sum = neighborSum(i,message,indicesM)
sum=0;
